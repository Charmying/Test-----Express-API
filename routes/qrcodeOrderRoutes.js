...
router.put('/orders/:id/complete', async (req, res) => {
  try {
    const order = await QRCodeOrder.findByIdAndUpdate(req.params.id, { status: 'completed' }, { new: true });
...
router.get('/reports', async (req, res) => {
  try {
    ...
    const totalRevenue = completedOrders.reduce((sum, order) => sum + order.items.reduce((subSum, item) => subSum + item.price, 0), 0);
